...
```bash
python3 ~/.Codex/skills/meta-ads-ratos/scripts/setup.py
```

### 2. Verificar .env

Checar se existe `~/.Codex/skills/meta-ads-ratos/.env`. Se NAO existir, criar com o template:
...
Depois que o `.env` estiver preenchido e o `setup.py` passar, o Codex DEVE proativamente guiar o cadastro de contas:
...
Esse fluxo conversacional e o jeito ideal de configurar — o usuario so responde as perguntas e o Codex preenche tudo.

## Cadastro de clientes (contas.yaml)

**Arquivo:** `~/.Codex/skills/meta-ads-ratos/contas.yaml`
...
Todos os scripts estao em `~/.Codex/skills/meta-ads-ratos/scripts/`. O padrao e:
...
**Arquivo:** `aprendizados.md` (na raiz da skill, `~/.Codex/skills/meta-ads-ratos/aprendizados.md`)
...
2. **Quando o usuário corrigir algo** (ex: "faltou o CTA", "tinha que ser carrossel", "botão errado"), o Codex DEVE perguntar:
...
O Codex DEVE ler este arquivo ANTES de criar qualquer campanha. Ele contem regras
aprendidas por tipo de campanha que evitam erros comuns (ex: carrossel sem preview,
ads bloqueados em posicionamentos, etc).

Se o tipo de campanha nao estiver documentado, o Codex DEVE primeiro buscar uma
