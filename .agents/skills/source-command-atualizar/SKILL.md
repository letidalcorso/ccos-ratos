---
name: "source-command-atualizar"
description: "Varre o estado atual do projeto e atualiza os arquivos de contexto que ficaram desatualizados. Compara o que existe nas pastas, skills e configurações com o que está documentado em AGENTS.md, _contexto/ e marca/design-guide.md. Use quando o usuário chamar /atualizar, quando disser \"atualiza o contexto\", \"os arquivos tão desatualizados\", \"sincroniza a memória\", ou no fim de uma sessão longa com muitas mudanças."
---

# source-command-atualizar

Use this skill when the user asks to run the migrated source command `atualizar`.
...
1. **Estrutura de pastas** — listar os diretórios de primeiro nível (ignorar `.git`, `node_modules`, `.Codex`, `templates`, `dados`)
2. **Skills instaladas** — listar `.Codex/skills/*/` e `.Codex/commands/*.md` (exceto este arquivo e os padrão: setup, iniciar, syncar, mapear)
3. **MCPs configurados** — verificar se `.Codex.json` ou `.Codex/mcp.json` existe e quais servers estão listados
