# Copy completa — site smartbase v2

Conjunto completo de textos pra todas as 7 telas + globais (header, footer, SEO).

**Regras de brand voice aplicadas:**
- Lowercase em headlines (lowercase é a marca, não preguiça)
- Tom confiante, direto, sem hype
- Garantia em linguagem **positiva** (lacrados + Apple 1 ano + 3 meses própria) — nunca defensiva
- WhatsApp como CTA recorrente
- Palavras **vetadas** que não aparecem em lugar nenhum: paralelo, ecossistema, diagnóstico, critério, "leva pra casa"
- Tese central: "o apple certo, não o mais caro"

**Status por bloco:**
- ✅ **FIRME** — escrito, on-brand, pronto pra usar
- 💭 **SUGESTÃO** — proposta forte, tua chamada se mantém ou ajusta
- ⚠️ **PRECISA TEU INPUT** — falta informação específica pra fechar

---

## GLOBAL

### Header (todas as páginas) ✅ FIRME (v2 — aprovado 04/06/2026)

```
[logo smartbase]   Vitrine   Sobre   Blog   FAQ        [quero meu apple →]
```

| Item | Texto | Status |
|---|---|---|
| Logo | smartbase (com o "•" azul entre smart e base) | ✅ |
| Nav | Vitrine · Sobre · Blog · FAQ | ✅ (item ativo destacado com vidro mais claro) |
| CTA | quero meu apple → | ✅ |

**Por quê "Vitrine":** "Catálogo" é genérico/transacional (Magalu/Mercado Livre). "Vitrine" tem conotação de seleção curada, presencial, Apple-feel. Casa com o tom premium do site e com o conceito de "loja particular".

### Footer (todas as páginas)

**Coluna 1 — marca** ✅ FIRME (v2 — sem alcance nacional)
```
[logo smartbase]
sua apple store particular.
novos sob encomenda e seminovos conferidos.
caxias do sul · serra gaúcha.
```

**Coluna 2 — Navegar**
- Vitrine
- Sobre
- Blog
- FAQ
- Contato

**Coluna 3 — Falar com a gente**
- WhatsApp →
- @smartbase.br
- caxias do sul, rs

**Linha legal (rodapé do rodapé):**
```
© 2026 smartbase · caxias do sul, rs          Privacidade · Termos
```

---

## 1. HOME

### Hero ✅ FIRME (v2 — aprovado 04/06/2026)

```
[imagem de desejo — Apple lifestyle, ambiente premium]

Tagline branca:    sua apple store.
Tagline azul:      particular.

Subtítulo:         novos e seminovos.
                   pra quem usa apple todo dia.

caxias do sul · serra gaúcha

CTA primário:      ver vitrine →
CTA secundário:    chamar no whatsapp →
```

**Por quê:**
- "sua apple store. particular." define marca em 4 palavras — "particular" é o twist (apple store é categoria, particular cria a marca)
- "novos e seminovos." entrega O QUE faz imediatamente (visitante novo bate o olho e entende em 3 segundos)
- "pra quem usa apple todo dia." indica PRA QUEM sem detalhe técnico — aspiracional, não defensivo
- Tirou "novos sob encomenda. seminovos em estoque. entrega em mãos." (processo técnico que tem dono em outros lugares — Vitrine, Sobre, FAQ)

### Posicionamento ✅ FIRME (v2 — aprovado 04/06/2026)

```
tem apple. e tem o apple certo pra você.
aqui a recomendação vem do uso real, não da ficha técnica.
```

**Por quê:**
- 1ª linha = afirmação sobre o universo Apple (não sobre a marca Smartbase). Cumpre o princípio do brand voice: abrir com verdade da categoria, marca entra depois
- "tem apple. e tem o apple certo pra você." separa categoria de escolha individual — força a pergunta "qual é o meu?", que é exatamente o que a Smartbase responde
- Pivotou de PREÇO ("raramente é o mais caro") pra ADEQUAÇÃO ("o certo pra você"). Sobe o tom de marca de "lugar barato" pra "lugar de escolha certa"
- 2ª linha (memory-blessed, aprovada anteriormente) traduz o **como**: uso real, não ficha técnica

### Diferenciais ✅ FIRME (v2 — aprovado 04/06/2026)

**Eyebrow:** por que a smartbase
**Título:** tem loja.
**Título azul:** e tem quem entende.

**4 cards (cada um com 1 job único, sem repetir os outros lugares do site):**

| # | Título | Descrição |
|---|---|---|
| 01 | loja física em caxias | tem endereço, tem balcão. dá pra vir pegar pessoalmente ou tirar dúvida olho no olho. |
| 02 | atendimento humano | sem chatbot, sem formulário. tu chama no whatsapp e quem responde entende de apple. |
| 03 | primeiro chega. depois paga. | o iphone chega na tua mão antes do dinheiro sair da tua conta. |
| 04 | só apple | especialização em uma marca muda o que a gente consegue te recomendar. uso real, não ficha técnica. |

**Por quê dessa reorganização (v2):**
- Cards anteriores (original garantido, preço, paga quando chega, foco apple) tinham **sobreposição** (01 e 04 diziam a mesma coisa) e **invadiam espaço** da Vitrine, Sobre→Garantia e Footer
- Headline "tem loja. e tem quem entende." prometia 2 coisas que os cards velhos NÃO entregavam — agora cards 01 e 02 provam diretamente o que a headline promete
- Card 03 mantém o ativo central "paga quando chega" sem o "única no Brasil" (ênfase forçada removida)
- Card 04 amarra com o Posicionamento (eco de "uso real, não ficha técnica")

**Promessas que saíram daqui e foram pros lugares delas:**
- Apple original / conferido → Sobre→Garantia
- Preço → cortado de diferencial (Magalu também tem preço; vira disputa fraca, não argumento de marca)
- Importação direta → Sobre→História (quando tiver)

### Produtos (faixa editorial) ✅ FIRME (v2 — aprovado 04/06/2026)

```
Eyebrow:    o que tem por aqui

Título:     o teu apple
Título azul: já tá te esperando.

(sem subtítulo — deixa a faixa visual respirar)

[faixa visual editorial — 4-6 produtos arranjados em
 lookbook/bento, NÃO em grade. iPhone Pro · iPhone Air ·
 Mac · iPad · Watch · AirPods (foto-âncora por categoria)]

CTA:        ver tudo na vitrine →
```

**Por quê:**
- "o teu apple já tá te esperando" cria desejo emocional sem listar specs
- A faixa visual entrega o O QUE (variedade) — texto não precisa repetir
- "Tu" mantém intimidade conversacional (callback do tom geral do site)
- Sai "linha 2025" (envelhece rápido) e "iphone novo em todas as cores" (promessa frágil)
- Sai "seminovos com garantia" (duplicava info do Sobre, do Footer e da Vitrine)
- CTA "ver tudo na vitrine →" tem o "tudo" como info de escopo, sem o "completo" enfático

### CTA final ✅ FIRME (v2 — aprovado 04/06/2026)

```
Título:        você já sabe o que quer.
Título azul:   falta só nos chamar.

Botão:         vou chamar
```

**Por quê:**
- Cortou o subtítulo "modelo certo, preço justo, resposta em minutos" — todas as 3 promessas já foram entregues nas seções anteriores (Posicionamento, Diferenciais, Faixa visual)
- "Preço justo" contradizia a decisão de cortar preço dos diferenciais (vira disputa fraca)
- "Resposta em minutos" era compromisso de SLA frágil (se demora 30 min, quebra)
- "Nos chamar" mantém sujeito coletivo (empresa), não cai no íntimo "me chamar" — Home é fechamento institucional, não atendimento individual
- Ritmo poético: "você já sabe" (3 sílabas) / "falta só nos chamar" (5 sílabas) — paralelismo rítmico

---

## 2. CATÁLOGO

### Hero ✅ FIRME (v2 — aprovado 04/06/2026)

```
[imagem-âncora — flat-lay editorial de produtos Apple ou cena de
 seleção curada, dark, sem repetir cenário Apple Store da Home]

Eyebrow:     vitrine
Breadcrumb:  Home > Vitrine

Headline:    o teu próximo está aqui.

Subline:     a apple inteira. particular tua.
             lacrada ou seminova. com garantia.
             e tu só paga quando ela tá na tua mão.

(sem CTA explícito no hero — as duas portas abaixo são a ação)
```

**Por quê:**
- "o teu próximo está aqui" ativa o usuário (declaração + posse) em vez de só rotular a página
- "particular tua" ecoa "sua apple store. particular." da Home → callback de marca explícito
- Subline cobre 4 fatos sem inflar: scope (apple inteira) + modalidade (lacrada/seminova) + garantia + ativo central (paga-quando-recebe)

### Duas portas — Novos vs Seminovos ✅ FIRME (estrutura A — aprovado 04/06/2026)

Logo abaixo do hero, **2 cards grandes lado a lado** (stacked em mobile):

```
┌─────────────────────────┬─────────────────────────┐
│ NOVOS                    │ SEMINOVOS                │
│                          │                          │
│ lacrados.                │ em estoque agora.        │
│ sob encomenda.           │ conferidos um a um.      │
│                          │                          │
│ garantia apple oficial   │ garantia smartbase       │
│ 12 meses                 │ 3 meses                  │
│                          │                          │
│ [ ver novos → ]          │ [ ver seminovos → ]      │
└─────────────────────────┴─────────────────────────┘
```

**Por quê:** decisão clara primeiro (cada modalidade tem promessa distinta), jornadas limpas, expectativa correta pra cada perfil. Em vez de misturar tudo e diluir a promessa, separa logo e mantém cada caminho coerente. Mais "Apple-feel" — Apple oficial não mistura "novo de fábrica" com "refurbished" na mesma vitrine.

### Filtros de categoria ✅ FIRME (após escolher modalidade)

```
[ todos ]  [ iPhone ]  [ Mac ]  [ iPad ]  [ Watch ]  [ AirPods ]
```

(Sticky no topo quando rola. Chip ativo tem vidro mais claro. Sem contagem numérica.)

### Card de produto (template lista vertical estilo Apple Shop) ✅ FIRME (v2)

**Estrutura:** cada produto = **faixa horizontal grande**, scroll vertical estilo Apple page. Foto de um lado, info do outro (alternando direita/esquerda a cada produto pra criar ritmo visual).

```
[FOTO GRANDE DO PRODUTO]   Nome:           iPhone 17 Pro
                            Descrição:      o mais completo da linha.
                                            pra quem quer o melhor iPhone,
                                            sem concessão.

                            Cor:            cosmic orange · navy · silver
                            Armazenamento:  256GB · 512GB · 1TB
                            Status:         em estoque  /  sob encomenda 7 dias

                            [CAMPO LIVRE editável por produto —
                             pra adicionar info extra quando precisar:
                             especificação, observação, badge, etc.]

                            Preço:          R$ 9.299 à vista
                                            (ou: "preço sob consulta")
                                            — sempre valor único, sem "x vezes"

                            CTA:            quero esse  →
```

**WhatsApp message pré-preenchida (template):**
```
"oi, quero o iPhone 17 Pro [cor] [armazenamento]. tudo certo?"
```

### CTA final ✅ FIRME (v2 — sem repetir "a gente")

```
Eyebrow:     não achou o teu?

Headline:    fala. eu acho.

Subline:     qualquer apple. se existe no mundo,
             chega aqui na tua mão.

Botão:       chamar no whatsapp →
```

**Por quê:** primeira pessoa direta ("eu acho") em vez de "a gente resolve / a gente consegue" repetidos. Mantém o tom confiante e cria intimidade — é o atendente falando, não a marca.

---

## 3. SOBRE

### Hero institucional ✅ FIRME (v2 — aprovado 04/06/2026)

```
[imagem ambiente Apple — workspace, mesa de café, ambiente premium, sem rosto]

Eyebrow:     sobre

Título:      anos vendendo Apple em Caxias.
Título azul: particular tua.

Subtítulo:   loja particular de Apple. novos importados,
             seminovos conferidos. todos com garantia.
```

**Por quê:**
- "particular tua." ecoa "sua apple store. particular." da Home (callback de marca)
- "novos importados, seminovos conferidos" corrige a imprecisão (não é "tudo importado" — só os novos)
- Saiu "agora pra todo o Brasil" (não tem alcance nacional ainda)

### História ⚠️ PRECISA TEU INPUT

Pra escrever a história forte sem foto/nome do dono, preciso de:

1. **Quando começou?** (ano de início)
2. **Como começou?** (importação pessoal pra amigo, primeiro pedido, mudou de carreira, etc.)
3. **Por que Apple?** (preferência pessoal, escolha estratégica, identificação com a marca)
4. **Sempre foi só Apple?** (ou já vendeu outros aparelhos)
5. **Algum marco notável?** (primeiro X pedidos, primeira loja física, momento de virada)

| por enquanto não vamos desenvolver uma página para isso, deixar um espaço na home talvez?

**Placeholder genérico até você me dar as respostas** (não usar como final, é só pro layout não ficar vazio):

```
desde [ANO], a smartbase vende Apple em caxias do sul. começou com
[CONTEXTO INICIAL] e cresceu sem perder o método: só Apple, importado
direto, com garantia que existe de verdade.

a regra é a mesma do primeiro pedido até hoje — recomendação por uso real,
sem empurrar o aparelho mais caro só pra fechar a venda. o iPhone que serve
pra você raramente é o topo de linha. e a gente diz isso na sua cara.

[MARCO RELEVANTE — primeira loja física, X mil clientes, lançamento de algum
produto que virou referência]

hoje a smartbase atende toda a serra gaúcha pessoalmente e o Brasil todo
pelo WhatsApp.
```

### Tese da marca ✅ FIRME (v2 — atualizada 04/06/2026)

```
tem apple. e tem o apple certo pra você.

aqui a recomendação vem do uso real, não da ficha técnica.
novo ou seminovo, o que fizer mais sentido pra você.
```

**Por quê:** versão V2 da tese-mestre. Pivotou de PREÇO ("raramente é o mais caro") pra ADEQUAÇÃO ("o certo pra você"). Mantém callback com Home/Posicionamento. Tirou menção a trade-in/"aparelho antigo como entrada" (descontinuado pra v1 com escala nacional).

### Garantia (bloco proeminente) ✅ FIRME (v2 — aprovado 04/06/2026)

```
Título:  garantia.

[2 COLUNAS — Novos vs Seminovos]

┌─────────────────────────────┬─────────────────────────────┐
│ NOVOS                        │ SEMINOVOS                    │
│                              │                              │
│ Apple oficial · 12 meses     │ smartbase · 3 meses          │
│                              │                              │
│ todo aparelho novo chega     │ pros seminovos, a smartbase  │
│ lacrado de fábrica.          │ adiciona 3 meses de          │
│ garantia internacional       │ garantia própria sobre o     │
│ Apple, válida em qualquer    │ aparelho.                    │
│ Apple Service no mundo,      │                              │
│ inclusive no Brasil.         │ acionamento direto com a     │
│                              │ smartbase, no whatsapp.      │
└─────────────────────────────┴─────────────────────────────┘
```

**Por quê:**
- 2 colunas (não 3) — uma por modalidade, espelha a estrutura das "2 portas" da Vitrine
- Título "garantia." (sem "que existe de verdade") — afirma sem se defender. Marca confiante não enfatiza, só apresenta
- Cortado "sem desvio de caixa" — jargão técnico que acende alarme em quem não conhece. Substituído por "lacrado de fábrica" (universalmente compreensível)
- Cortado "importado direto" — info que tem dono no Sobre→História (princípio: 1 promessa = 1 dono)
- Tom positivo: afirma o que É, não o que NÃO é

### Loja física ⚠️ PRECISA TEU INPUT

```
loja física em caxias do sul, rs.

[ENDEREÇO COMPLETO — rua, número, bairro, CEP]
[HORÁRIO DE FUNCIONAMENTO — dias da semana + horários]
[TELEFONE FIXO — se houver, além do WhatsApp]

atendemos a serra gaúcha pessoalmente. [PLACEHOLDER — preencher alcance regional/nacional depois conforme expansão]
```

### CTA WhatsApp ✅ FIRME (v2 — aprovado 04/06/2026)

```
Título:      antes de fechar, chama.
Subtítulo:   atendimento direto. sem chatbot, sem formulário.
Botão:       abrir whatsapp →
```

**Por quê:**
- "antes de fechar, chama" substitui "pra qualquer coisa, chama a gente" (genérico demais)
- Captura o momento de decisão: visitante leu Sobre, considera comprar — esse é o convite certo
- Concreto + ativo + curto (3 palavras + verbo de ação)

---

## 4. BLOG (index)

### Hero ✅ FIRME (mantém aprovado v1, leve ajuste)

```
Eyebrow:     blog

Título:      dicas e novidades
Título azul: do mundo Apple.

Sub:         conteúdo por quem vive Apple há anos. comparativos, guias e
             o que importa pra você escolher certo.
```

### Filtros ✅ FIRME

```
[ todos ]  [ iPhone ]  [ seminovo ]  [ garantia ]  [ comparativos ]
```

### Card de post (template) ✅ FIRME

```
[COVER IMAGE]

Categoria:      iPhone
Data:           28 de maio de 2026 · 6 min de leitura
Título:         vale a pena comprar iPhone seminovo?
Excerpt:        depende de 3 coisas — e a smartbase é honesta sobre cada uma.
                aqui está o checklist...
[ler mais →]
```

### Estado vazio (caso não tenha posts) ✅ FIRME (aprovado v1)

```
✦
em breve.
novidades e conteúdo Apple chegando por aqui.
```

---

## 5. POST DE BLOG (template)

### Breadcrumb ✅ FIRME (template)

```
Home > Blog > [Título do post]
```

(Schema `BreadcrumbList` no HTML pra rich snippet do Google.)

### Cover image
Imagem grande no topo. Cada post define a sua.

### Título + meta ✅ FIRME (template)

```
[TÍTULO DO POST]

[Data por extenso]  ·  [N min de leitura]  ·  [Categoria]
```

### Conteúdo
Variável por post. Escrevo quando tivermos a pauta de cada um.

**4 posts iniciais sugeridos** (já mapeados): | validado com o levantamento de seo?
1. vale a pena comprar iPhone seminovo? *(maior leverage SEO)*
2. como saber se o iPhone é original
3. iPhone importado é seguro? tudo sobre garantia internacional Apple no Brasil
4. iPhone 17 vs iPhone 16: qual escolher em 2026

### CTA WhatsApp no fim do conteúdo 💭 SUGESTÃO

```
─────────────────────────────────────

pronto pra dar o próximo passo?

chama no whatsapp que a gente recomenda o iPhone certo pra você —
levando em conta uso real, orçamento e o que tem disponível.

[abrir whatsapp →]
```

### Posts relacionados ✅ FIRME (template)

```
você também pode gostar

[card relacionado 1]  [card relacionado 2]  [card relacionado 3]
```

---

## 6. FAQ

### Hero ✅ FIRME

```
Eyebrow:     perguntas frequentes

Título:      ainda em dúvida?
Título azul: a gente responde.

Sub:         tudo o que mais perguntam sobre comprar Apple aqui.
```

### Categorias e perguntas 💭 SUGESTÃO (revisa com o dono)

Sugiro 5 categorias com perguntas iniciais. Revisa pra ver o que mais é perguntado na loja física e adiciona/corta.

#### Garantia

**Como funciona a garantia Apple no Brasil?**
> Todo iPhone novo importado vem com **garantia internacional Apple de 1 ano**, válida em qualquer Apple Authorized Service Provider no mundo — **inclusive no Brasil** (sim, dá pra acionar na Apple BR sem problema). O período começa a contar da data de fabricação.

**O que está incluído nos 3 meses de garantia própria da Smartbase?**
> Pros aparelhos **seminovos**, a smartbase adiciona 3 meses de garantia própria sobre o aparelho. Cobre defeitos de funcionamento (tela, bateria, carregamento, áudio). Não cobre dano por queda, contato com líquido ou uso indevido. Acionamento direto comigo pelo WhatsApp.

**E se o aparelho der problema depois da garantia?**
> A smartbase continua sendo seu contato. A gente ajuda a resolver — seja indicando o melhor service autorizado, seja ajudando a contestar reparo com a Apple. Cliente da smartbase não fica na mão.

#### Entrega

**Em quanto tempo chega o iPhone novo?**
> Sob encomenda em **até 7 dias úteis** após pagamento confirmado. Pra seminovos em estoque, **2-3 dias úteis** dependendo da região.

**Vocês entregam pra qualquer cidade do Brasil?**
> Sim, qualquer cidade com CEP. Envio rastreado.

**Quanto custa o frete?**
> Varia por região. Te passamos o valor exato no WhatsApp antes de fechar.

**Como rastreio o pedido?**
> Assim que o aparelho sai, te enviamos o código de rastreio direto no WhatsApp.

#### Pagamento

**Quais formas de pagamento vocês aceitam?**
> Pix (com 5% de desconto), cartão de crédito (em até 12x), boleto, transferência. Pra parcelamento longo no cartão, pode haver juros — confirmamos no atendimento.

**Posso parcelar?**
> Sim, em até 12x no cartão de crédito (sujeito a juros da operadora depois da 6ª parcela).

**Vocês emitem nota fiscal?**
> Sim, sempre. Nota fiscal pessoa física ou pessoa jurídica.

#### Trade-in

**Vocês aceitam meu iPhone antigo como entrada?**
> Fazemos avaliação caso a caso. **Chama no WhatsApp** que a gente te conta na hora se faz sentido e quanto vale o seu aparelho.
>
> [abrir WhatsApp →]

#### Originalidade e procedência

**Como sei que o iPhone é original?**
> Todo aparelho vendido pela smartbase é Apple original e lacrado de fábrica. Você pode verificar no site oficial da Apple usando o número de série antes de aceitar. Sempre acompanhado de nota fiscal.

**De onde vêm os aparelhos importados?**
> Importação direta com nota fiscal de entrada. Lacrados, embalagem original, garantia Apple ativa.

**Posso ativar o iPhone no Brasil sem problema?**
> Sim, sem problema algum. iPhone é desbloqueado de fábrica e funciona com qualquer operadora brasileira. A garantia Apple também é válida normalmente.

### CTA WhatsApp ✅ FIRME

```
─────────────────────────────────────

ainda tem dúvida?

[chama no whatsapp →]
```

---

## 7. CONTATO

### Hero ✅ FIRME

```
Título:  onde estamos.
Sub:     loja física em caxias do sul. e atendendo todo o Brasil pelo WhatsApp.
```

### Mapa + endereço (2 colunas) ⚠️ PRECISA TEU INPUT

**Coluna esquerda:**
```
[MAPA GOOGLE EMBEDADO — usar lat/long da loja]
```

**Coluna direita:**
```
[NOME DA LOJA]
[ENDEREÇO COMPLETO — rua, número, complemento, bairro]
[CIDADE, UF — CEP]

Horário:
[seg-sex: __h às __h]
[sáb: __h às __h]
[dom: fechado]

[TELEFONE FIXO se houver]
WhatsApp: +55 54 9 9662-4494
```

### CTA WhatsApp dominante ✅ FIRME

```
mais rápido que vir até aqui:
chama no whatsapp.

resposta em minutos. atendimento direto.

[abrir whatsapp →]
```

---

## SEO — title tags e meta descriptions

### Title tags (até 60 chars)

| Página | Title |
|---|---|
| Home | `iPhone original em Caxias do Sul · smartbase` |
| Catálogo | `Catálogo Apple — iPhone, Mac, iPad · smartbase` |
| Sobre | `Sobre · smartbase` |
| Blog | `Blog · smartbase — apple por quem vive apple` |
| Post | `[Título do post] · smartbase` |
| FAQ | `Perguntas frequentes · smartbase` |
| Contato | `Onde estamos · smartbase` |

### Meta descriptions (até 160 chars)

| Página | Description |
|---|---|
| Home | `Apple original com garantia da gente. iPhone novo sob encomenda e seminovo avaliado, atendimento direto no WhatsApp. Loja física em Caxias do Sul.` |
| Catálogo | `Catálogo Apple completo: iPhone 17, Mac, iPad e acessórios. Original, com garantia, pedido pelo WhatsApp e entrega em todo o Brasil.` |
| Sobre | `Smartbase — anos vendendo Apple em Caxias do Sul, agora pra todo o Brasil. Loja particular, atendimento direto, garantia que existe de verdade.` |
| Blog | `Conteúdo Apple por quem vive Apple: comparativos, dicas de compra e o que importa pra você escolher o iPhone certo.` |
| FAQ | `Tira suas dúvidas sobre comprar Apple na Smartbase: garantia, entrega, formas de pagamento e originalidade.` |
| Contato | `Smartbase em Caxias do Sul, RS. Loja física + atendimento WhatsApp pra todo o Brasil. Veja onde estamos e fale com a gente.` |

---

## O que precisa de ti pra fechar 100%

| # | Item | Onde aparece | Tipo |
|---|---|---|---|
| 1 | **História do dono** (ano de início, como começou, por que Apple, marcos) | Sobre → bloco história | Conversa de 10 min — me responde as 5 perguntas |
| 2 | **Endereço completo + horário + telefone fixo** (se houver) | Sobre + Contato | Info simples |
| 3 | **Lat/long da loja** (pro mapa Google embedado) | Contato | Abre Google Maps, clica no endereço, copia as coordenadas |
| 4 | **Revisão da lista de FAQ** | FAQ | Lê as 12 perguntas que escrevi, corta/adiciona o que o dono mais ouve |
| 5 | **Pauta + conteúdo dos posts do blog** | Posts | Vamos por post — me confirma a primeira pauta ("vale a pena seminovo?") e eu escrevo |
| 6 | **Lat/long da loja** + foto da fachada | Contato + Sobre (v2) | v2 — não bloqueia lançamento |

**Sem isso, tudo o que está marcado ⚠️ vira placeholder.** O resto (60% do site) já está firme e pode ser dropado no Figma assim que tu acabar o design system.
