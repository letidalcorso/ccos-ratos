...
- `templates/` — templates de skills prontos pra personalizar com /figma
- `dados/` — arquivos para análise (CSV, PDF, etc)
- `scheduled/` — agendamentos recorrentes do Claude (morning-brief, daily-meme, ftemper-tendencias, etc.)
- `scripts/` — scripts de automação (publish-postforme.js, whatsapp-zapi.js)
- `Letícia Particular/` — projetos pessoais da Letícia (widgets Scriptable, capas Notion, setup iPhone)
- `templates/ferramentas/catalogo.md` – APIs e ferramentas disponiveis pra usar skills
...
Claude e Codex trabalham em conjunto. Regras de quando o Codex entra:

**Resgate automático — invocar `/codex:rescue` quando:**
- Um comando Bash falhar 2 ou mais vezes seguidas na mesma tarefa
- A tarefa técnica estiver bloqueada sem solução clara após 2 tentativas
- O erro for de sintaxe complexa (Liquid, JS assíncrono, CSS avançado)

**Revisão de código — invocar `/codex:rescue` para segunda opinião quando:**
- Editar arquivos `.liquid` para a Smartbase (risco de quebrar o tema)
- Escrever JS com lógica de negócio relevante
- Construir HTML/CSS de proposta ou landing page que vai para o cliente

**Nunca interromper o fluxo de conteúdo/marketing para Codex** — textos, pautas, copies, carrosséis, slides e estratégias são 100% Claude, Codex não entra.
